 ...
 */
    ...
    {
        private static readonly int VENDOR_ID = 773; // Safecontrol Vendor ID
        private static readonly string VENDOR_STRING = "Safecontrol s.r.o.";
        ...
        public LocalDevice(int deviceId, Transport transport, string deviceName, string modelName)
        {
...
//AdK - DCC
public void sendGlobalBroadcast(UnconfirmedRequestService serviceRequest, bool initiated)
{
    transport.sendUnconfirmed(Address.GLOBAL, null, serviceRequest, true, initiated);
}
...
{
    return this.transport.getNetwork().getLocalNetworkNumber();
}

//AdK - support for DeviceCommunicationControl
private EnableDisable enableDisable = EnableDisable.enable;
private java.util.GregorianCalendar endTime = null;
//private java.org.free.com.serotonin.bacnet4j.org.free.bacnet4j.util.concurrent.

public EnableDisable getDCCEnableDisable()
{
    // check if disable is still active
    if (endTime != null && (new java.util.GregorianCalendar()).after(endTime))
    {
        setDCCEnableDisable(EnableDisable.enable);
        endTime = null;
    }
...
public void setDCCEnableDisable(EnableDisable ed)
{
    System.out.println("DCC state changed: " + ed);
    enableDisable = ed;
}

public java.util.GregorianCalendar getDCCEndTime()
{
    return endTime;
}

public void setDCCEndTime(java.util.GregorianCalendar end)
{
    endTime = end;
}
    }
